...
export default function BlockRenderer({ block, ctx }: { block: Block; ctx: BlockRenderContext }) {
  ...
  switch (block.type) {
    case "section": {
      ...
      return (
        <section id={block.props.id} className={`px-6 ${PADDING_CLASS[block.props.paddingY] ?? PADDING_CLASS.md}`} style={{ backgroundColor: bg, color }}>
    ...
    case "heading": {
      ...
      return <Tag className={`${HEADING_SIZE[level]} ${ALIGN_CLASS[block.props.align] ?? ""}`}>{block.props.text}</Tag>;
    }

    case "text":
      return block.props.html ? <p className={`leading-relaxed ${ALIGN_CLASS[block.props.align] ?? ""}`}>{renderRichText(block.props.html)}</p> : null;

    case "image":
      return block.props.url ? (
        <div className="aspect-video bg-neutral-100 rounded-xl overflow-hidden w-full">
