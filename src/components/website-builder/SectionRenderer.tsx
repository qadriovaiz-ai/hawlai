    ...
    case "hero":
      return (
        <section className="px-6 py-16 sm:py-24 text-center" style={{ backgroundColor: theme.dark }}>
          ...
          {section.subheadline && <p className="text-base sm:text-lg max-w-xl mx-auto mb-6 opacity-80" style={{ color: theme.bg }}>{renderRichText(section.subheadline)}</p>}
          {section.ctaText && (
    ...
    case "text":
      return (
        <section className="px-6 py-12 max-w-2xl mx-auto text-center">
          ...
          <p className="text-neutral-600 leading-relaxed">{renderRichText(section.body)}</p>
        </section>
      );
    case "image_text":
      return (
          ...
          <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-5">
            {(section.items ?? []).map((item: any, i: number) => (
              <div key={i} className="p-5 rounded-xl border border-neutral-200">
                ...
                <p className="text-sm text-neutral-500">{renderRichText(item.description)}</p>
              </div>
            ))}
          </div>
        </section>
          ...
          <div className="grid sm:grid-cols-2 gap-5">
            {(section.items ?? []).map((item: any, i: number) => (
              <div key={i} className="p-5 rounded-xl bg-neutral-50">
                <p className="text-neutral-700 italic mb-2">&ldquo;{renderRichText(item.quote)}&rdquo;</p>
          ...
          <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-6">
            {(section.items ?? []).map((item: any, i: number) => (
              <div key={i} className="text-center">
                ...
                <p className="text-sm text-neutral-500">{renderRichText(item.bio)}</p>
              </div>
            ))}
          </div>
        </section>
          ...
          <div className="space-y-4">
            {(section.items ?? []).map((item: any, i: number) => (
              <div key={i}>
                ...
                <p className="text-sm text-neutral-500">{renderRichText(item.answer)}</p>
              </div>
            ))}
          </div>
        </section>
