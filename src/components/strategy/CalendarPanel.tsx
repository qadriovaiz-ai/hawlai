...
/** Where "Create this" goes: Content Marketing, with the format and the idea as its topic. */
export function createHref(w: Week): string | null {
      ...
      {quarter && (
        <>
          ...
          <ol className="space-y-2">
            {quarter.weeks.map((w) => {
              const href = createHref(w);
              return (
                <li key={w.week} className="border border-slate-200 rounded-lg p-3 space-y-1.5">
                  <div className="flex items-center justify-between gap-2 flex-wrap">
                    <p className="text-[11px] font-semibold text-slate-500 tabular-nums">Week {w.week} · {day(w.starts)} – {day(w.ends)}</p>
