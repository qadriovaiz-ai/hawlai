...
export async function GET() {
  ...
  if (logo) {
    try { doc.image(logo.buffer, doc.page.width - 50 - 100, 45, { fit: [100, 40] }); } catch { /* malformed image — skip, never break the report */ }
  }
  ...
  doc.fillColor("#000").fontSize(14).text("Health Score", { continued: true }).fontSize(14).fillColor(accentColor).text(`  ${growth.healthScore}/100`);
  ...
  doc.moveDown(2);
  doc.fontSize(8).fillColor("#999").text(reportFooterText(branding), { align: "center" });
