...
export async function POST(request: Request) {
  ...
  try {
    ...
    const uploadRes = await metaPost(`act_${process.env.META_AD_ACCOUNT_ID}/adimages`, {
    ...
    const creativeRes = await metaPost(`act_${process.env.META_AD_ACCOUNT_ID}/adcreatives`, {
    ...
    // Step 5: campaign
    const campaignRes = await metaPost(`act_${process.env.META_AD_ACCOUNT_ID}/campaigns`, {
    ...
    const targeting = cityKey
      ? { geo_locations: { cities: [{ key: cityKey, radius: 25, distance_unit: "km" }] }, age_min: 21, age_max: 55 }
      : { geo_locations: { countries: ["IN"] }, age_min: 21, age_max: 55 };

    const adsetRes = await metaPost(`act_${process.env.META_AD_ACCOUNT_ID}/adsets`, {
    ...
    const adRes = await metaPost(`act_${process.env.META_AD_ACCOUNT_ID}/ads`, {
      name: plan.headline,
